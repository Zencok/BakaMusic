...
assert.match(playerStyles, /#components--modal-base-container:has\(> \.modal--mv-player\)[\s\S]*?background:\s*transparent !important;[\s\S]*?backdrop-filter:\s*blur\(1px\) !important/);
assert.match(playerStyles, /html\[data-native-video-overlay="true"\][\s\S]*?&::before[\s\S]*?background:\s*transparent;[\s\S]*?backdrop-filter:\s*blur\(1px\)/);
assert.match(playerStyles, /html\[data-native-video-overlay="true"\][\s\S]*?#components--modal-base-container:has\(> \.modal--mv-player\)[\s\S]*?backdrop-filter:\s*none !important/);
assert.doesNotMatch(playerStyles, /var\(--appOverlayMask\)/);
