...
assert.match(buildWorkflow, /npm run native:install/);
assert.match(buildWorkflow, /NATIVE_REPO_TOKEN/);
assert.doesNotMatch(buildWorkflow, /NATIVE_SOURCE_REF/);
assert.doesNotMatch(buildWorkflow, /npm run build:native/);
assert.ok(fs.existsSync(path.join(root, "scripts/native-modules-manifest.json")));
assert.ok(fs.existsSync(path.join(root, "scripts/install-native-modules.cjs")));
assert.ok(fs.existsSync(path.join(root, "scripts/update-native-modules-manifest.cjs")));
assert.ok(fs.existsSync(path.join(root, ".github/workflows/native-update.yml")));
const nativeUpdateWorkflow = read(".github/workflows/native-update.yml");
assert.match(nativeUpdateWorkflow, /update-native-modules-manifest/);
assert.match(nativeUpdateWorkflow, /NATIVE_REPO_TOKEN/);
assert.doesNotMatch(nativeUpdateWorkflow, /2\.3\.1/);
const installNativeSource = read("scripts/install-native-modules.cjs");
assert.match(installNativeSource, /localModulesMatchManifest/);
assert.doesNotMatch(installNativeSource, /2\.3\.1/);
const taglibNativeSource = read("src/common/taglib-native.ts");
assert.doesNotMatch(taglibNativeSource, /2\.3\.1/);
