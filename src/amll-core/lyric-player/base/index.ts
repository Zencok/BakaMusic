	...
	implements HasElement, Disposable
{
	...
	/**
	 * 设置未高亮歌词内容的亮度基线，取值范围为 0 到 1。
	 */
	setInactiveBrightness(value = 0.2): void {
		this.inactiveBrightness = Math.max(0, Math.min(1, value));
	}

	/** 获取未高亮歌词内容的亮度基线。 */
	getInactiveBrightness(): number {
	...
	/** 设置间奏圆点是否在非对唱布局中水平居中。 */
	setCenterInterludeDots(center = true): void {
		...
		this.currentLyricGroups.forEach((group, i) => {
			...
			if (!setDots && shouldShowDots) {
				...
				let targetX = this.centerInterludeDots
					? Math.max(
						0,
						(this.size[0] - this.layoutState.interludeDotsSize[0]) / 2,
					)
					: 0;
