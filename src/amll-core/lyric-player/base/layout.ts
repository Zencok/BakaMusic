 ...
 * 计算一行歌词在当前布局中的模糊等级。
 *
 * 当前播放区域附近的歌词保持清晰，超过起始距离后才逐渐增加模糊；
...
export function computeLineBlur(input: ComputeLineBlurInput): number {
	...
	const lineDistance =
		itemIndex < scrollToIndex
			? Math.abs(scrollToIndex - itemIndex)
			: Math.abs(itemIndex - Math.max(scrollToIndex, latestIndex));
	const blurStartDistance = isCompact ? 1 : 2;
	const effectiveBlurDistance = Math.max(
		0,
		lineDistance - blurStartDistance,
	);
	...
	const blurLevel = 0.65 + effectiveBlurDistance * 0.85;
