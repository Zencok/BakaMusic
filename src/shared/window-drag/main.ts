...
const isPointNearWindowEdge = (
    point: ICommon.IPoint,
    size: ICommon.ISize,
) => (
    point.x <= RESIZE_EDGE_THRESHOLD
    || point.y <= RESIZE_EDGE_THRESHOLD
    || point.x >= size.width - RESIZE_EDGE_THRESHOLD
    || point.y >= size.height - RESIZE_EDGE_THRESHOLD
);
...
const makeWin32WindowFullyDraggable = (
    ...
    browserWindow.hookWindowMessage(
        ...
        (wParam: Buffer, lParam: Buffer) => {
            ...
            browserWindow.setPosition(cachePosition.x, cachePosition.y, false);
        },
    );
};
